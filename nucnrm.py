...
def nrmapp(A, B, C = None, d = None, G = None, h = None): 
    ...
    p, q = B.size
    if p < q:
        raise ValueError('row dimension of B must be greater than or equal to column dimension')
    
    if type(A) not in (matrix, spmatrix) or A.typecode is not 'd' or \
        A.size[0] != p*q:
        raise TypeError('A must be a real dense or sparse matrix with p*q rows if B has size (p, q)')
    n = A.size[1]
    ...
    if type(G) not in (matrix, spmatrix) or G.typecode is not 'd' or \
        G.size != (m, n):
        raise TypeError('G must be a real dense matrix or sparse matrix of size (m, n) if h has length m and A has n columns')
       
    if C is None: C = spmatrix(0.0, [], [], (n,n))
    if d is None: d = matrix(0.0, (n, 1))
    if type(C) not in (matrix, spmatrix) or C.typecode is not 'd' or \
        C.size != (n,n):
        raise TypeError('C must be real dense or sparse matrix of size (n, n) if A has n columns')
    if type(d) is not matrix or d.typecode is not 'd' or d.size != (n,1):
        raise TypeError('d must be a real matrix of size (n, 1) if A has n columns')
...
def checksol(sol, A, B, C = None, d = None, G = None, h = None): 
    ...
    if sol['status'] is 'optimal':
        ...
        print('Dual residual: ', blas.nrm2(res))

        if m:
           print('Minimum primal slack (scalar inequalities): ', min(h - G*sol['x']))
           print('Minimum dual slack (scalar inequalities): ', min(sol['z']))
        ...
        if p:
            ...
            nrmZ = max(s)
            print('Norm of Z: ', nrmZ)
            print('Nuclear norm of A(x) + B: ', nrmX)
            print('Inner product of Z and A(x) + B: ', blas.dot(sol['Z'], X))
